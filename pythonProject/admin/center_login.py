...
from admin import center_dashboard


class CenterLogin:
    def __init__(self):
        self.root = Tk()
        self.root.title("CENTER LOGIN")
        ...
        self.bg_image_path = "C:/Users/manjo/Downloads/gvhjv.jpg"
        ...
        # Login Button
        self.loginButton = Button(self.canvas, text="Login", width=15, font=self.font, bg=self.buttonColor, fg=self.mainColor, command=self.check_center)
    ...
    def update_background(self):
        ...
        text_overlay = "CENTER LOGIN"
        self.canvas.create_text(650, 100, text=text_overlay, font=('Bahnschrift', 40, 'bold'), fill="white", anchor=CENTER)

    def check_center(self):
        email = self.txt1.get()
        password = self.txt2.get()
        q = f"SELECT * FROM add_center WHERE email = '{email}' and password='{password}'"
        self.cr.execute(q)
        ...
        if result is None:
            ...
            self.root.destroy()
            center_dashboard.CenterDashboard(centerDetails=result)
